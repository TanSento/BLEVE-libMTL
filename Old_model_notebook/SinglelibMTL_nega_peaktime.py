...
def main(params):
    print(params)
    X_train_torch, X_test_torch, target_train, target_test, quantile = get_data(mode='single', task_name='nega_peaktime')

    data_train = BLEVEDatasetSingle(X_train_torch, target_train, task_name='nega_peaktime')
    data_test = BLEVEDatasetSingle(X_test_torch, target_test, task_name='nega_peaktime')


    train_loader = DataLoader(data_train, batch_size=512, shuffle=False)  
    ...
    num_out_channels = {'nega_peaktime': 1}
