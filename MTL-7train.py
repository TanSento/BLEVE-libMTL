...
def main(params):
    ...
    data_train = BLEVEDataset(X_train_torch, target_train, sev_tar=True)
    data_test = BLEVEDataset(X_test_torch, target_test, sev_tar=True)
    ...
    task_dict = {
                'posi_peaktime': {'metrics':['MAPE'], 
                                'metrics_fn': MAPE(quantiles[0]),
                                'loss_fn': HuberLoss(),
                                'weight': [0]}, 
                'nega_peaktime': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[1]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]},
                'arri_time': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[2]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]},
                'posi_dur': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[3]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]},
                'nega_dur': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[4]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]},           
                'posi_pressure': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[5]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]},
                'nega_pressure': {'metrics':['MAPE'], 
                            'metrics_fn': MAPE(quantiles[6]),
                            'loss_fn': HuberLoss(),
                            'weight': [0]}
                }
    ...
    num_out_channels = {'posi_peaktime': 1, 'nega_peaktime': 1, 'arri_time': 1, 'posi_dur': 1, 'nega_dur': 1,
                        'posi_pressure': 1, 'nega_pressure': 1}
    ...
    BLEVENet.train(train_loader, test_loader, 500, mode = "multitask", name=params.name)
